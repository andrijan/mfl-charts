...
class Api:
    ...
    def __init__(self, year):
        ...
        """
        self.mfl_export_url = (
            'http://football.myfantasyleague.com/{}/export'.format(year)
        )
    ...
    def _export(self, params):
        ...
        Used by all other methods inside the API class to create a correctly
    ...
    def players(self, players=None, since=None, details=False):
        ...
        All other functions use player ID's, so call this function if
        information like names, teams, or positions are needed.

        Args:
            players (List[int]): Player ID's to return data for.
            since (str): Date from which the function should return updates
                         Should be formatted as mm/dd/yyyy.
            details (bool): If True, returns extra information including
                            weight, birthdate, draft year, and ID on other
        ...
        if since is not None:
            timestamp = util.convert_to_timestamp(since)
            params['SINCE'] = timestamp
        if players is not None:
            params['PLAYERS'] = util.concat(players)
        return self._export(params)

    def all_rules(self):
        """Return all scoring rules supported by MFL.

        Contains information about whether a scoring rule is scored for
        players, coaches, and/or teams. Also contains the abbreviation for
        each rule that is returned as part of the `rules` function.
        """
    ...
    def injuries(self, week=None):
        ...
        Returns player ID, status (Out, Probable, Suspended etc.), and details.
        Also contains timestamp showing when this information was most
        recently updated.

        Args:
            week (int): NFL week to get injury data for. If no value is
                        provided, the most recent week is used.
        """
    ...
    def nfl_schedule(self, week=None):
        ...
        Returns information about each game for a particular week including
        kickoff time, teams involved, spread,
        offense/defense rankings, and time remaining.

        Args:
            week (int): NFL week to get schedule information for.
            If no value is provided, the most recent week is used.
        """
    ...
    def adp(self,
        ...
        Args:
            franchises (int): Number of franchises in leagues for which ADP
                              data is desired. For example, if ADP data for
            ...
            time (str): Only returns data from drafts conducted after the given
                        date. Should be formatted as mm/dd/yyyy.
            days (int): Only returns data from drafts conducted within the last
                        n days, where n in the integer provided.
        """
        params = {'TYPE': 'adp'}
        ...
        if is_keeper:
            ...
            params['IS_KEEPER'] = 0
        if time is not None:
            params['TIME'] = util.convert_to_timestamp(time)
        if days is not None:
            params['DAYS'] = days
        return self._export(params)

    """
    Actual AAV API call only supports FRANCHISES, but could eventually use the
    other AAV endpoint:
    ...
    def aav(self, franchises=None):
        ...
        Args:
            franchises (int): Number of franchises in leagues for which AAV
                              data is desired. For example, if AAV data for
                              10-team leagues is wanted, pass in the number 10.
    ...
    def top_adds(self, week=None):
        ...
        Only players that have been added in more than 2%% of leagues will be
        returned.

        Args:
            week (int): NFL week to get information for. If no value is
                        provided, the most recent week is used.
    ...
    def top_drops(self, week=None):
        ...
        Only players that have been dropped in more than 2%% of leagues will
        be returned.
    ...
    def top_starters(self, week=None):
        ...
        Only players that have been started in more than 1%% of leagues will be
        returned.
    ...
    def league(self, league_id, franchise_id=None, password=None):
        ...
        Returns information like league name, roster sizes, and starting lineup
        requirements. If a valid password
        and franchise_id combination are supplied, it also returns otherwise
        private information about the league
    ...
    def rosters(self, league_id, franchise_id=None):
        ...
        Response includes players status (active, IR, taxi) and
        salary/contract information for that player if
        the given league supports those functions.

        Args:
            league_id (int): 5-digit league ID
            franchise_id (str): 4-digit franchise ID. If provided, returns
                                roster information for just this franchise
        """
    ...
    def league_standings(self, league_id):
        ...
        Response descriptions:
            ...
            divpf: Divisional Points For (Total Year-to-Date Points Scored In
                   Divisional Games)
            confw: Conference Wins
            confl: Conference Losses
            conft: Conference Ties
            confpf: Conference Points For (Total Year-to-Date Points Scored In
                    Conference Games)
            pf: Points For (Total Year-to-Date Point Scored)
            pa: Points Against (Total Year-to-Date Opponent Points Scored)
            avgpa: Average Points Against
                   (Weekly Average Opponent Points Scored)
            ...
            all_play_w: The number of all-play wins for a franchise, assuming
                        a franchise played all other franchises each week of
                        the season
            all_play_l: The number of all-play losses for a franchise,
                        assuming a franchise played all other franchises each
                        week of the season
            all_play_t: The number of all-play ties for a franchise, assuming
                        a franchise played all other franchises each week of
                        the season
        """
    ...
    def weekly_results(self, league_id, week=None):
        ...
        Includes scores for all starter and non-starter players for all
        franchises.

        Args:
            league_id (int): 5-digit league ID
            week (int/str): NFL week to get information for. If no value is
                            provided, the most recent week is used. Also
                            supports "YTD" for year-to-data data
        """
    ...
    def live_scoring(self, league_id, week=None, details=False):
        ...
        Includes each franchise's current score, how many game seconds
        remaining that franchise has, players who have yet to play, and
    ...
    def player_scores(self,
        ...
        Args:
            ...
            week (int/str): NFL week to get information for. If no value is
                            provided, the most recent week is used. Also
                            supports "YTD" for year-to-date data or
                            "AVG" for weekly average scores.
            players (List[int]): Player ID's to return data for
            count (int): Number of results to return
            position (str): Position to return results for
            status (str): Supports 'freeagent' to only return scores for
                          free agents
        """
    ...
    def transactions(self,
        ...
        Args:
            league_id (int): 5-digit league ID
            transaction_type (str): Kinds of transactions to return. Supported
                                    values are "waiver", "bbid_waiver",
                                    "trade", "ir", "taxi",
                                    "bbid_waiver_request", "survivor_pick",
                                    and "pool_pick"
            franchise_id (str): 4-digit franchise ID to return transactions for
            days (int): Limit results to transactions done within a certain
                        number of days
    ...
    def rss(self, league_id):
        ...
        Includes League standings, current week's live scoring, last week's
        fantasy results, and the five newest message board topics
    ...
    def projected_scores(self,
                         ...
                         position=None,
                         status=None):
        """Return expected fantasy points for certains players in a given
        ...
        Args:
            ...
            count (int): Number of results to return
            position (str): Position to return results for
            status (str): Supports 'freeagent' to only return scores for free
                          agents
        """
        params = {
    ...
    def league_search(self, search_query):
        ...
        Case-insensitive search query must contain at least three characters.
        Note that the search string can either be
        the league name, the league commissioner's email address, or an
    ...
    def message_board(self, league_id, count=10):
        ...
        Args:
            league_id (int): 5-digit league ID
            count (int): Number of message board threads to return.
                         Default is 10.
        """
    ...
    def message_board_thread(self, league_id, thread_id):
        ...
        Args:
            league_id (int): 5-digit league ID
            thread_id (int): ID of message board thread to return information
                             for
        """
        params = {
    ...
    def player_profile(self, players):
        ...
        Args:
            players (List[int]): List of players ID's to return information
                                 about
        """
        params = {
            'TYPE': 'playerProfile',
            'P': util.concat(players)
        }
        return self._export(params)
    ...
    def who_should_i_start(self,
                           ...
                           week=None,
                           players=None):
        """Return site-wide "Who Do I Start?" data, comparing pairs of
        players at the same position.

        Shows what percentage of all MFL users would choose one player over
        another.

        Args:
            league_id (int): 5-digit league ID
            franchise_id (str): Restrict comparisons to only players on given
                                franchise's roster
    ...
    def appearance(self, league_id):
        """Return skin, home page tabs, and modules within each tab set up by
        the commissioner.
